import "./css/Header.css"
import Modal from 'react-bootstrap/Modal';
import bull from '../assets/bull.png'
const Header = ({show, onHide, cl, showStats, onHideStats, clStats}) =>{

    const info = <img src="data:image/svg+xml;base64,"></img>
    
    return(
    <div className="header">
        <div className="info header-item" onClick={cl}>
                {info} <span className="text-title">How to play</span>
        </div> 

        <div className="stats header-item" onClick={clStats}>
            {<img src={bull} width={"29px"} height={"29px"}/>} <span className="text-title st">Stats</span> 
        </div>
        
        <Modal
            show={show}
            onHide={onHide}
        />
        <Modal
            show={showStats}
            onHide={onHideStats}
        />

        
        
    </div>
    );
}

export default Header;